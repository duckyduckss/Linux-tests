...
-rw-r--r-- 1 user2  user2   5120 10 apr 08:00 file.txt  
lrwxrwxrwx 1 root   root       7 10 apr 08:00 link -> file.txt  
```
...
d. Bitul setgid pe director nu are niciun efect; se aplică doar fișierelor executabile.
...
b. Toate fișierele și directoarele din interior devin executabile.
c. Directorul respectiv va ignora bitul setuid – pe Linux acesta nu produce efect pe directoare.
...
c. Procesul și-a terminat execuția, dar încă are o intrare în tabelul de procese, așteptând ca părintele să preia codul său de terminare.
...
c. Codul de eroare al ultimei comenzi executate.
...
d. Există exact o partiție de swap în listare.
...
1234567 -rw-r--r-- 2 student student 1000 07 mai 14:00 file.txt  
...
Care afirmații despre fișierele `file.txt` și `hardlink.txt` sunt adevărate? *(2 răspunsuri corecte)*

a. `hardlink.txt` și `file.txt` referă același inode pe disc, deci ambele denumiri indică același fișier fizic.
b. Ștergerea fișierului `file.txt` va lăsa în continuare conținutul disponibil prin `hardlink.txt`.
c. `hardlink.txt` este un link simbolic către `file.txt`.
d. Modificarea conținutului prin `hardlink.txt` nu va afecta fișierul `file.txt`.
...
b. Sunt două denumiri pentru același dispozitiv fizic.
c. `/dev/sda1` este primul disc din sistem, iar `/dev/sda` al doilea.
...
**20.** Comanda `uname -a` afișează informații despre sistemul în execuție. Din exemplul de mai jos:
...
b. Cele două comenzi rulează în paralel (concurent), `comanda1` producând date pe care `comanda2` le prelucrează pe măsură ce le primește.
c. `comanda2` va începe execuția **doar după** ce `comanda1` și-a încheiat complet execuția.
...
a. Ghilimelele duble permit interpretarea (expansiunea) variabilelor și secvențelor speciale în interior, pe când ghilimelele simple **nu** permit interpretarea acestora.
...
d. Ghilimelele duble sunt folosite doar pentru a cita texte care conțin ghilimele simple.
...
$ file libdata.so  
libdata.so: ELF 64-bit LSB shared object, x86-64, dynamically linked ...  
```
